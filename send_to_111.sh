...
mkdir deploy
rsync -a --no-links --exclude-from 'd_excludes.txt' "$path" deploy
chmod 755 -R deploy
...
rsync --no-links --exclude-from 'd_excludes.txt' -e "ssh -p 22" -avvzcWP --port=22 deploy/ "$_SERVER111_USER@$_SERVER111_IP:/home/$_SERVER111_USER/scripts/$folderpath"
